...
- Nav: "Solution Architecture" rail with tabs:
  - Estimating (legacy route `/planning` maps to `/sa/estimating`)
  - Requirements
  - HLD
  - ARM Catalog
  - ERD
...
Note: Roadmap and Licensing are surfaced under the Solution Architecture rail for quick access in the design workflow.
...
- Quick effort planning with category/component/complexity and T‑shirt sizing.
- Estimates derived from configurable size→hours mapping (localStorage).
- CSV export via native Save dialog where supported; falls back to download.
- Route: `/sa/estimating` (legacy: `/planning`).
...
### End‑User Hardware Constraints

- __Default to lightweight paths__: Mock mode is the default; local LLM is strictly optional.
- __Local LLM guidance__: If using Ollama, prefer small models (e.g., 7–8B) on machines with ≥16 GB RAM; avoid GPU assumptions. Provide clear fallback to mock/remote.
- __Graceful degradation__: Disable streaming on slow CPUs/network; show progress with cancellable jobs; surface token and cost estimates before run.
- __Chunked processing__: For large CSV/XLSX, parse in chunks (web workers where possible) to keep UI responsive and memory bounded.
- __No WebGPU dependency__: Do not require WebGPU/WebGL for core flows; diagrams render client‑side but should fall back to non‑accelerated paths.
- __Offline‑friendly__: Reads/writes and cached AI results work offline; provider keys stored locally (`web/src/lib/secrets.ts`).
